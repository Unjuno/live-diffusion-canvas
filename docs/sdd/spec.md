...
→ 気に入らない局所解を Noise Brush で reject する
→ rejected region の不確定性が上がり、別の局所解が探索される
...
重要：Noise Brush は Human Layer を局所適用するための仕組みではない。Human Layer は全体または runtime session に対する継続的な guide であり、Noise Brush は現在の局所解を reject して別解探索を促す負の介入である。
...
Noise Brush は、Generated Image 上で指定した領域の **現在の局所解を reject する** ためのブラシである。
...
実 backend では、`noiseMask` を latent 解像度へ変換し、該当領域に局所的な uncertainty injection を行い、1〜3 step だけ denoise を進める実装を優先する。
...
この値は、Noise Brush で指定された rejected region にどれだけ強く不確定性を再注入するかを表す。全体画像用の別 noise strength は v0.1 では定義しない。
...
`noiseMask` は「ここに Human Layer を適用する」という意味ではない。

`noiseMask` は「この領域の現在の局所解は reject された」という意味を持つ。runtime はこの情報を使って局所不確定性を上げ、現在の局所解から離れた別解を探索する。その探索に対して Prompt、Human Layer、周辺 runtime state が条件として作用する。
...
| CFG | Prompt の強さ | 3.0 |
| Noise Strength | rejected region への不確定性注入の強さ | 0.25 |
| Seed | 乱数固定 | ON |
